  ...
  cat "${secrets_list_file}" | while IFS= read -r line; do
    if [[ "$line" =~ ^[a-zA-Z].*= ]]; then
      ...
      if [[ "$url" =~ ^blob: ]]; then
        ...
        value="$(aws ssm get-parameter --name "${key}" --with-decryption --query "Parameter.Value" --output text)" || failed=1
        echo -n "${value}" > "${dest}"  # -n to make sure a newline isn't added to the value
      fi
      if [[ -n "$failed" ]]; then
